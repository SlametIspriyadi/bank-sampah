...
<div class="p-6 sm:p-8 bg-white rounded-2xl shadow-lg max-w-2xl mx-auto">
    <h2 class="text-2xl font-bold text-gray-800 mb-1">Edit Jenis Sampah</h2>
    <p class="text-gray-500 mb-8">Perbarui detail untuk jenis sampah: <span class="font-semibold">{{ $sampah->jenis_sampah }}</span></p>
    ...
    <form action="{{ route('admin.sampah.update', $sampah->sampah_id) }}" method="POST" id="sampah-form">
        @csrf
        @method('PUT')
        <div class="space-y-6">
            {{-- Jenis Sampah --}}
            <div>
                ...
                <input type="text" name="jenis_sampah" id="jenis_sampah" class="w-full px-4 py-2.5 text-base text-gray-900 bg-gray-50 border border-gray-300 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:bg-white @error('jenis_sampah') border-red-500 focus:ring-red-500 @enderror" value="{{ old('jenis_sampah', $sampah->jenis_sampah) }}" maxlength="100" required>
                @error('jenis_sampah')
            ...
            <div>
                ...
                <select name="satuan" id="satuan" class="w-full px-4 py-2.5 text-base text-gray-900 bg-gray-50 border border-gray-300 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:bg-white @error('satuan') border-red-500 focus:ring-red-500 @enderror" required>
                    ...
                    <option value="Kg" {{ old('satuan', $sampah->satuan) == 'Kg' ? 'selected' : '' }}>Kg (Kilogram)</option>
                    <option value="Pcs" {{ old('satuan', $sampah->satuan) == 'Pcs' ? 'selected' : '' }}>Pcs (Pieces/Buah)</option>
                </select>
                @error('satuan')
                    <p class="text-sm text-red-600 mt-1.5">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="harga" id="harga" class="w-full pl-10 pr-4 py-2.5 text-base text-gray-900 bg-gray-50 border border-gray-300 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:bg-white @error('harga') border-red-500 focus:ring-red-500 @enderror" value="{{ old('harga', $sampah->harga) }}" min="0" required>
                </div>
                @error('harga')
                    <p class="text-sm text-red-600 mt-1.5">{{ $message }}</p>
                @enderror
